...
#data loading
plantdoc_data = ImageFolder(root="../data/PlantDoc-Dataset", transform=test_transformation)
...
def model_eval(weight_file_path):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:          #begins loop pf batched images
            ...
            correct += (preds == labels).sum().item()       #compares prediciton to actual label and counts correct
            total += labels.size(0)
